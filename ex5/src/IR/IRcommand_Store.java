...
public class IRcommand_Store extends IRcommand
{
	...
	// Constructor for storing a TEMP's value to a stack offset
	...
	public void staticAnalysis() {
		...
		if (!is_stack_store) { // Register move: Check src, define dst
			...
			// it modifies memory state. Liveness of TEMPs isn't directly affected by the store itself,
	...
	public void MIPSme() {
		...
		if (srcRegNum < 0) {
			System.err.printf("Warning: Source TEMP %s for Store (%s) has no register. Static analysis might be incomplete or value unused? Skipping MIPSme.\n", src, var_name);
		...
		if (is_stack_store) {
			...
			if (dstRegNum < 0) {
				System.err.printf("Warning: Destination TEMP %s for Store (move, var=%s) has no register. Skipping MIPSme.\n", dst, var_name);
				return;
			}
			gen.move(dst, src);
		}
	...
	public HashSet<TEMP> liveTEMPs() {
		...
		// Liveness definition: TEMPs whose values are needed *before* this instruction overwrites them.
